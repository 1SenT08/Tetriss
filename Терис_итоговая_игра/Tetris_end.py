...
class Start_logo(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__(fon_sprite)
...
class Lose_pic(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__(fon_sprite)
        self.image = load_image('фон_поражение.png')
        self.rect = self.image.get_rect()
        self.mask = pygame.mask.from_surface(self.image)
...
def main():
    ...
    while running:
        ...
        if flag_loss:
            pos_x_figure = size[0] // 2 - 2 * size_square
            pos_y_figure = size[1] - 22 * size_square
            flag = False
            figur_ver = 0
            hold_figur = 0
            hold_figur_for_sprite = -1
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:  # движение фигуры влево и вправо
                if not flag_loss and event.key == pygame.K_RIGHT and tetris.examination_bottom_coords(figure, pos_x_figure + size_square, # Проверяет, можно ли фигуре переметится впарво
                ...
                elif event.key == pygame.K_SPACE and not_abuse_flag:
                    ...
                    if flag_loss:
                        if timer != '0':
                            f = open('data/board.txt', 'a')
                            f.write('-----------------------------\n')
                            f.write(' player name | score  | time \n')
                            f.write(f' player1     | {str(score).ljust(7)}| {timer} \n')
                            f.write('-----------------------------\n')
                            f.close()
                        figure = tetris.view_figure()
                        next_figure = tetris.view_figure()
